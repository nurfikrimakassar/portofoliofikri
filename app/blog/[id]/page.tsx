  ...
  return (
    <div className="relative min-h-screen bg-[#0a0a0a] text-[#f5f5f5] font-sans overflow-x-hidden">
      ...
      <header className="relative z-10 max-w-[1040px] mx-auto px-8 pt-[132px] max-[640px]:px-6 max-[640px]:pt-[104px]">
        <Link href="/blog" className="font-mono text-[12px] text-[#737373] no-underline hover-link">
          ← ~/fikri / blog
        </Link>
        <div className="flex flex-wrap gap-3 items-center my-7 font-mono text-[11px] tracking-[0.1em] text-[#737373]">
          <span className="px-[11px] py-[5px] border border-white/18 text-[#d4d4d4]">{post.cat.replace(/^\/\/\s*/, "")}</span>
          <span>{post.date}</span>
          <span className="text-[#404040]">·</span>
          <span>{post.read}</span>
        </div>
        <h1 className="text-[clamp(32px,5vw,54px)] font-bold tracking-[-0.035em] leading-[1.05]">{post.title}</h1>
        <p className="text-[clamp(17px,2vw,21px)] leading-[1.6] text-[#a3a3a3] mt-6">{post.excerpt}</p>
      </header>
      ...
      <div className="relative z-10 max-w-[1040px] mx-auto px-8 max-[640px]:px-6">
        ...
        />
        <article className="flex flex-col gap-6 text-[17.5px] leading-[1.8] text-[#d4d4d4] pb-16">
          <BlockBody body={DB.body || []} />
        </article>
      </div>

      <footer className="relative z-10 max-w-[1040px] mx-auto px-8 pb-10 max-[640px]:px-6">
        <div className="flex justify-between items-center flex-wrap gap-4 font-mono text-[11px] text-[#525252] pt-6 border-t border-white/[0.08]">
